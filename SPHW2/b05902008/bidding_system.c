...
void end_auction(int *busyhost, int *score, int *fd, int *cnt){
  char RESULT[1000];
  int player[4], rank[4];
  read(*fd,RESULT,1000);
...
int main(int argc, char *argv[]){
  ...
  if(playernum<4 || playernum>20){
    fprintf(stderr,"constraint: 4 <= player_num <= 20 not satisfied\n");
    exit(1);
  }
  ...
  while(plays!=cnt){
    ...
    for(int i = 0;i<hostnum;i++){
      if(FD_ISSET(fd[i][2],&doset)){
        end_auction(&busyhost[i],score,&fd[i][2],&cnt);
      }
    }
    for(int i = 0;i<hostnum;i++){
      if(last>=plays)
        break;
      if(busyhost[i]==0){
        sprintf(pipemessage,"%d %d %d %d\n",matches[last][0],matches[last][1],matches[last][2],matches[last][3]);
